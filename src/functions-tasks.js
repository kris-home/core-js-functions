...
function memoize(func) {
  const cachedFirstReturn = func();
  return () => cachedFirstReturn;
}
...
function logger(func, logFunc) {
  return (...args) => {
    const string = args.map((arg) => JSON.stringify(arg)).join(',');
